                        ...
                        <Grid item xs={12} sm={3} py={2} px={1}>
                            {
                                insuranceCategoryList?.map((category: ICategory) => {
                                    return (
                                        <Box component={'div'} key={category.id} sx={{ my: 2, px: 2, py: 1, border: '1px solid grey', borderRadius: '4px', textAlign: 'center', cursor: 'pointer' }} onClick={() => router.push(`/insurance?category=${category.slug}`)}>
